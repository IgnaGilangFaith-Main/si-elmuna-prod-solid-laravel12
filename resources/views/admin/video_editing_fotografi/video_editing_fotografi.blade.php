...
@section('content')
    ...
    <div class="my-3">
        <a href="{{ url('/data_video_editing_fotografi/terhapus') }}" class="btn btn-secondary">Restore Data</a>
    </div>
    <hr>
    <div class="col-12 col-sm-8 col-md-4">
        <label for="" class="mb-2">Cari Data</label>
        <form action="{{ url('/data_video_editing_fotografi') }}" method="get">
            <div class="input-group">
                ...
                <a href="{{ url('/data_video_editing_fotografi') }}" class="btn btn-danger">Batal</a>
            </div>
        </form>
    </div>
    <hr>
    <div class="col-md-6">
        ...
        <form action="{{ url('/data_video_editing_fotografi/filter') }}" method="get">
            <div class="input-group">
    ...
    <div class="col-md-6">
        ...
        <form action="{{ url('/data_video_editing_fotografi/export') }}" method="post">
            @csrf
            <div class="input-group">
                <span class="input-group-text">Dari Tanggal : </span>
                <input type="date" class="form-control" name="tgl_awal" required>
                ...
                @if (count($data) > 0)
                    @foreach ($data as $datum)
                        <tr>
                            ...
                            <td>
                                <center>
                                    <a href="{{ url('/edit_video_editing_fotografi/' . $datum->id) }}"
                                        class="btn btn-warning">Edit</a>
                                    <a href="{{ url('/hapus_video_editing_fotografi/' . $datum->id) }}"
                                        class="btn btn-danger my-2">Hapus</a>
                                    <a href="{{ url('/sertifikat/tambah/video-editing-fotografi/' . $datum->id) }}"
