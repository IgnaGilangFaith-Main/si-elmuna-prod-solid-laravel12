...
class BahasaInggrisController extends Controller
{
     ...
     */
    public function store(TambahBahasaInggrisRequest $request)
    {
        $validated = $request->validated();
        $validated['paket'] = $this->prosessPaket($validated['paket']);
        BahasaInggris::create($validated);
    ...
    public function index(Request $request)
    {
        ...
        if (! empty($cari)) {
            ...
            $data = BahasaInggris::latest()->paginate(25);
        }
     ...
     */
    public function update($id, EditBahasaInggrisRequest $request)
    {
        $validated = $request->validated();
        ...
        $bahasaInggris->update($validated);
     ...
     */
    public function deletedBahasaInggris()
    {
    ...
    {
        BahasaInggris::withTrashed()->where('id', $id)->restore();
    ...
    {
        $data = BahasaInggris::withTrashed()->findOrFail($id);
    ...
    {
        BahasaInggris::withTrashed()->findOrFail($id)->forceDelete();
    ...
    public function export(Request $request)
    {
        ...
        $baseQuery = BahasaInggris::orderBy('id', 'desc')
            ->whereDate('created_at', '>=', $start_date)
            ->whereDate('created_at', '<=', $end_date);
        ...
        $no = 1;
        $rows = 2;
        $filename = 'Laporan Daftar Peserta Bahasa Inggris '.$start_date.' sampai '.$end_date.'.xlsx';
