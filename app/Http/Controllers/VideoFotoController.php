...
class VideoFotoController extends Controller
{
    ...
    public function store(TambahVideoFotoRequest $request)
    {
    ...
    public function index(Request $request)
    {
        ...
        if (! empty($cari)) {
            $data = VideoFoto::orderBy('id', 'desc')
            ...
            $data = VideoFoto::latest()->paginate(25);
        }
    ...
    public function update($id, EditVideoFotoRequest $request)
    {
        $validated = $request->validated();
        ...
        $videoFoto = VideoFoto::findOrFail($id);
        $videoFoto->update($validated);
    ...
    public function deletedVideoFoto()
    {
    ...
    {
        VideoFoto::withTrashed()->where('id', $id)->restore();
    ...
    {
        $data = VideoFoto::withTrashed()->findOrFail($id);
    ...
    {
        VideoFoto::withTrashed()->findOrFail($id)->forceDelete();
    ...
    public function export(Request $request)
    {
        ...
        $baseQuery = VideoFoto::orderBy('id', 'desc')
            ->whereDate('created_at', '>=', $start_date)
            ->whereDate('created_at', '<=', $end_date);
        ...
        $no = 1;
        $rows = 2;
        $filename = 'Laporan Daftar Peserta Video Editing & Fotografi '.$start_date.' sampai '.$end_date.'.xlsx';
