...
use App\Http\Requests\EditPublicSpeaking;
use App\Http\Requests\TambahPublicSpeakingRequest;
...
class PublicSpeakingController extends Controller
{
    /**
    ...
    {
        return view('pendaftaran.public_speaking');
    }

    /**
     * Menyimpan data pendaftaran baru
     */
    public function store(TambahPublicSpeakingRequest $request)
    {
        ...
        PublicSpeaking::create($validated);

        sweetalert()->success('Anda ['.$validated['nama'].'] Berhasil Mendaftar!');

        return redirect('/daftar_public_speaking');
    }
    ...
    public function index(Request $request)
    {
        ...
        if (! empty($cari)) {
            $data = PublicSpeaking::orderBy('id', 'desc')
            ...
            $data = PublicSpeaking::latest()->paginate(25);
        }

        return view('admin.public_speaking.public_speaking', ['data' => $data]);
    }
    ...
    public function edit($id)
    {
        ...
        return view('admin.public_speaking.edit-public-speaking', ['data' => $data]);
    }

    /**
     * Memperbarui data yang sudah ada
     */
    public function update($id, EditPublicSpeaking $request)
    {
        $validated = $request->validated();
        ...
        $publicSpeaking = PublicSpeaking::findOrFail($id);
        $publicSpeaking->update($validated);

        sweetalert()->success('Update Data Berhasil!');

        return redirect('/data_public_speaking');
    }
    ...
    public function delete($id)
    {
        ...
        return view('admin.public_speaking.hapus-public-speaking', ['data' => $data]);
    }
     ...
     */
    public function deletedPublicSpeaking()
    {
        $data = PublicSpeaking::onlyTrashed()->get();

        return view('admin.public_speaking.data-terhapus', ['data' => $data]);
    }
    ...
    {
        PublicSpeaking::withTrashed()->where('id', $id)->restore();
    ...
    {
        $data = PublicSpeaking::withTrashed()->findOrFail($id);

        return view('admin.public_speaking.hapus-permanen', ['data' => $data]);
    }
    ...
    {
        PublicSpeaking::withTrashed()->findOrFail($id)->forceDelete();

        sweetalert()->success('Berhasil Hapus Data Secara Permanen!');

        return redirect('/data_public_speaking/terhapus');
    }
    ...
    public function export(Request $request)
    {
        ...
        $baseQuery = PublicSpeaking::orderBy('id', 'desc')
            ->whereDate('created_at', '>=', $start_date)
            ->whereDate('created_at', '<=', $end_date);
        ...
        $no = 1;
        $rows = 2;
        $filename = 'Laporan Daftar Peserta Public Speaking '.$start_date.' sampai '.$end_date.'.xlsx';
