...
class PemasukanController extends Controller
{
    ...
    /**
     * Mendapatkan data pemasukan yang sudah difilter
     */
    private function getFilteredData($filter, $paginate = false)
    {
        ...
        if ($dateRange) {
            ...
            if ($filter === 'hari_ini' || $filter === 'kemarin') {
                $query = Masuk::whereDate('created_at', $startDate);
            } elseif (in_array($filter, ['bulan_ini', 'bulan_lalu'])) {
                $query = Masuk::whereMonth('created_at', $startDate->month)
                    ->whereYear('created_at', $startDate->year);
            } elseif (in_array($filter, ['tahun_ini', 'tahun_lalu'])) {
                $query = Masuk::whereYear('created_at', $startDate->year);
            } else {
                $query = Masuk::whereBetween('created_at', [$startDate, $endDate]);
            }
        } else {
            $query = Masuk::query();
        }
    ...
    private function calculateTotal($filter)
    {
        ...
        return $data->sum('jumlah_pemasukan');
    }
    ...
    private function setExportHeaders($sheet)
    {
        ...
        $sheet->setCellValue('C1', 'Tanggal Pemasukan');
    ...
    private function fillExportData($sheet, $data, $total)
    {
        ...
        foreach ($data as $item) {
            ...
            $sheet->setCellValue('B'.$row, $item->ket_pemasukan);
            $sheet->setCellValue('C'.$row, $item->created_at->format('d/m/Y'));
            $sheet->setCellValue('D'.$row, $item->jumlah_pemasukan);
            $row++;
        }
    ...
    /**
     * Menampilkan data pemasukan dengan filter
     */
